...
import os
import scipy
from util import load_images,pyr,fft_nxcorr,shotgun
...
def test_classe(classe,templates_path='/media/tom/TOSHIBA EXT/PIR',fullpath='/media/tom/TOSHIBA EXT/visceral/volumes/CTce_ThAb'):
    ...
    with open(str(classe)+'.csv', 'w') as csvfile:
        ...
        for imagename in os.listdir(fullpath):
            ...
            pyramid = tuple(pyramid_gaussian(image.get_fdata(), downscale=2, multichannel=False))
            max_layer = len(pyramid) - 1
            ind = pyr(pyramid=pyramid, template=template.get_fdata(), layer=max_layer -3,max_recursion=max_layer-3)
...
def display_lung_test():
    ...
    display_square(image,template.shape,right_center_1302_100)
...
def routine_2d():
    ...
    t_dep = time.clock()
    pyramid = tuple(pyramid_gaussian(image, downscale=2, multichannel=False))
    max_layer = len(pyramid) - 1
    # template_rotate= np.rot90(template.get_fdata(), k=1)
    maxind = pyr2d(pyramid=pyramid, template=template, layer=max_layer - 3, max_recursion=max_layer - 3)
